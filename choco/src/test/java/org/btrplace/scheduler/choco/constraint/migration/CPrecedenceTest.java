...
/**
 * Created by vkherbac on 26/01/15.
 */
public class CPrecedenceTest {
    ...
    public void test() {
        ...
        // Register the Precedence constraint
        ps.getConstraintMapper().register(new CPrecedence.Builder());
        ...
        cstrs.add(new Precedence(vms.get(1), vms.get(0)));
        ...
        try {
            ...
            // Verify that vm#1 terminate before vm#0
            int end_vm1 = -2, start_vm0 = -1;
            for (Action a : p.getActions()) {
                if (a.toString().contains("migrate")) {
                    if (a.toString().contains("vm#1")) { end_vm1 = a.getEnd(); }
                    if (a.toString().contains("vm#0")) { start_vm0 = a.getStart(); }
                }
            }
            Assert.assertTrue(end_vm1 >= start_vm0);
