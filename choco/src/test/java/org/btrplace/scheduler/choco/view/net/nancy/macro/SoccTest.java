...
public class SoccTest {
    ...
    @Test
    public void decommissioning() throws SchedulerException,ContradictionException {
        ...
        // Define power values
        int powerIdleNode = 110, powerVM = 16, powerBoot = 20;
        ...
        List<Switch> srcSwitches = new ArrayList<>();
        for (int i=0; i<nbSrcNodes/nbNodesRack; i++) {
            srcSwitches.add(net.newSwitch());
        }
        List<Switch> dstSwitches = new ArrayList<>();
        for (int i=0; i<nbDstNodes/nbNodesRack; i++) {
            dstSwitches.add(net.newSwitch());
        }
        int nbSrcSwitches = 0;
        for (Switch srcSwitch : srcSwitches) {
            srcSwitch.connect(srcNodesBw, srcNodes.subList(nbNodesRack*nbSrcSwitches,nbNodesRack*(nbSrcSwitches+1)));
            nbSrcSwitches++;
        }
        int nbDstSwitches = 0;
        for (Switch dstSwitch : dstSwitches) {
            dstSwitch.connect(dstNodesBw, dstNodes.subList(nbNodesRack*nbDstSwitches,nbNodesRack*(nbDstSwitches+1)));
            nbDstSwitches++;
        }
        for (Switch srcSwitch : srcSwitches) { swMain.connect(backbone, srcSwitch); }
        for (Switch dstSwitch : dstSwitches) { swMain.connect(backbone, dstSwitch); }
        mo.attach(net);
