...
#p <- p + scale_colour_manual(values=df\$VMLabel)
#p <- p + scale_fill_manual(values=df\$VMLabel)
...
#p
ggsave(p, file = "./actions.eps", width = 7, height = 7, device=cairo_ps)
